...
public class CharacterController2D : MonoBehaviour
{
	...
	[SerializeField] private float m_HurtForce = 200f;                          // Amounf of force added when the player is hurt by enemy
	[SerializeField] private ParticleSystem dustFX;								// Particle System for dust
	...
	[SerializeField] private AudioClip m_jumpFX;                                // Sound effects for jump
	...
	public void Move(float move, bool crouch, bool jump)
	{
		...
		//Trigger move animation based on move input
		m_anim.SetFloat("Speed", Mathf.Abs(move));

	}
	
	public void TakeHit(Vector3 _direction)
	{
		m_Rigidbody2D.AddForce(_direction * m_HurtForce);
		m_anim.Play("Hurt");
		//Create dustFX
		CreateDust();
	}
	...
	public void Jump(float _jumpForce)
	{
		...
		m_anim.Play("Jump");
		m_SoundManager.PlayOneShot(m_jumpFX);

		//Create dustFX
		CreateDust();
	}

	public Vector3 GetOppositeDir()
	{
		if (m_FacingRight)
		{
	...
	private void Flip()
	{
		...
		m_chatBubble.transform.localScale = theScale;
	}
